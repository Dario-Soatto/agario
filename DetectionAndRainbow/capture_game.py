...
def main():
    ...
    with mss.mss() as sct:
        if len(sct.monitors) > 2:
            ...
            padding_x = int(ext['width'] * 0.1)
            padding_y = int(ext['height'] * 0.1)
            monitor = {
                "top": ext['top'] + padding_y,
                "left": ext['left'] + padding_x,
                "width": ext['width'] - (2 * padding_x),
                "height": ext['height'] - (2 * padding_y)
            }
            print(f"Using external monitor: {ext['width']}x{ext['height']} at ({ext['left']}, {ext['top']})")
            print(f"Capture region: {monitor['width']}x{monitor['height']} with 10% padding")
        else:
            mon = sct.monitors[1]
            padding_x = int(mon['width'] * 0.1)
            padding_y = int(mon['height'] * 0.1)
            monitor = {
                "top": padding_y,
                "left": padding_x,
                "width": mon['width'] - (2 * padding_x),
                "height": mon['height'] - (2 * padding_y)
            }
